define(require => {
  ...
  const answerAnnotationsRendering = require('./annotations-rendering')
  const keys = require('./grading/keys')
    ...
    setupAnnotationEditing: (
      ...
      function setupAnnotationRemoval($answers, gradingUriPrefix) {
        $answers.on('mousedown', '.remove-annotation-area', event => {
          ...
          removeAnnotation(
            $(event.target).closest('.answerAnnotation'),
            gradingUriPrefix
          )
        })
        ...
        function removeAnnotation($annotationElem, gradingUriPrefix) {
          ...
          const removedAnnotationIndex = $answerText
            .find('.answerAnnotation')
            .index($annotationElem)
      ...
      function setupAnnotationAddition($containerElement, gradingUriPrefix) {
          ...
          .filter(range => {
            if (selectionHasNothingToUnderline(range)) {
              ...
              return (
                ...
                ($container.hasClass('answerText') ||
                  $container.parents('div.answerText').size() > 0)
              )
            }
          })
        ...
        $containerElement
          ...
          .filter(
            e =>
              !$(e.target).closest('.add-annotation-text').length &&
              !$(e.target).closest('.annotation-message').length
          )
          .merge(
            $containerElement
              .asEventStream('keyup')
              .filter(e => e.keyCode === keys.ESC)
          )
          .onValue(() => {
        ...
        function selectionHasNothingToUnderline(range) {
          ...
          const hasImages = _.contains(
            _.toArray(contents.children).map(x => x.tagName),
            'IMG'
          )
          return contents.textContent.length === 0 && !hasImages
        }

        function preventDragSelectionFromOverlappingCensorAnswerText(
          $containerElem
        ) {
          if (isCensor()) {
        ...
        function annotationPopup(range) {
          ...
          const mergedAnnotations = answerAnnotationsRendering.mergeAnnotation(
            answerAnnotationsRendering.get($answerText),
            annotationPos
          )
          answerAnnotationsRendering.renderGivenAnnotations(
            $answerText,
            mergedAnnotations
          )

          const $popup = $(
            `<div class="add-annotation-popup"><input class="add-annotation-text" type="text" value="${renderedMessages}"/><i class="fa fa-comment"></i><button data-i18n="arpa.annotate"></button></div>`
          ).i18n()
        ...
        function addAnnotation(gradingUriPrefix) {
          ...
          function add(
            ...
            const annotations = answerAnnotationsRendering.get($answerText)
              ? answerAnnotationsRendering.mergeAnnotation(
                  answerAnnotationsRendering.get($answerText),
                  newAnnotation
                )
              : [newAnnotation]
        ...
        function calculatePosition($answerText, range) {
          ...
          const charactersBefore = charactersBeforeContainer(
            range.startContainer,
            range.startOffset
          )
          const charactersUntilEnd = charactersBeforeContainer(
            range.endContainer,
            range.endOffset
          )
          return {
          ...
          function charactersBeforeContainer(rangeContainer, offset) {
            ...
            const container = containerIsImg
              ? rangeContainer.childNodes[offset]
              : rangeContainer
            const offsetInside = containerIsImg ? 0 : offset
            const nodesBeforeContainer = _.takeWhile(
              answerNodes,
              node => node !== container
            )
            return offsetInside + _.sum(nodesBeforeContainer.map(x => (x.nodeType === Node.TEXT_NODE ? x.textContent.length : x.nodeName === "IMG" ? 1 : 0)))
          }
        }
      }
    },
  ...
  function setupAnnotationDisplaying($answers) {
    ...
    function popupAlreadyShownForCurrentAnnotation(event) {
      const mouseOverAnnotationElem = $(event.target).closest(
        '.answerAnnotation'
      )[0]
      const popupsCurrentAnnotationElem = $(
        '.remove-annotation-popup:visible'
      ).closest('.answerAnnotation')[0]
      return (
        event &&
    ...
    function renderRemovePopup(event) {
      const $annotation = $(event.target)
      const $popup = popupWithMessage(
        $annotation,
        $annotation.attr('data-message')
      )
    ...
    function popupWithMessage($annotation, message) {
      const $messageContainer = $(
        '<span class="remove-annotation-popup"><span class="annotation-message"></span></span>'
      )
      $messageContainer.find('.annotation-message').text(message || '-')
      if (isAllowedToRemoveAnnotation($annotation)) {
        $messageContainer.append(
          '<i class="fa fa-times-circle remove-annotation-area"></i>'
        )
      }
    ...
    function isAllowedToRemoveAnnotation($annotation) {
      ...
      function isCensorAndOwnAnnotation() {
        return (
          isCensor() && $annotation.closest('.answerText').hasClass('is_censor')
        )
      }
    }
    ...
    function mouseOffsetX(mousemove) {
      ...
      const annotationLeftOffsetFromPageEdge =
        $(annotation).offsetParent().offset().left + annotation.offsetLeft
