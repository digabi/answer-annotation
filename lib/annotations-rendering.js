define(require => {
  ...
  function createRangeFromMetadata($answerText, annotation) {
    ...
    const nodeTextLengths = _.map(
      nodes,
      node => (isImg({ node }) ? 1 : node.textContent.length)
    )
    ...
    const accumulators = _.zipWith(nodes, accumLengths, (node, length) => ({
      node,
      length
    })).map((node, index) => ({
      node: node.node,
    ...
    const startOffset = isImg(startObject)
      ? startObject.index
    ...
    const endOffset = isImg(endObject)
      ? endObject.index
      : nodeContentLength(endObject) -
        (endObject.length - (annotation.length + offset))
  ...
  function getOverlappingMessages($answerText, start, length) {
    ...
    const parted = getOverlappingAnnotations(currentAnnotations, {
      startIndex: start,
      length: length
    })
  ...
  function mergeAnnotation(annotations, newAnnotation) {
    ...
    if (parted.overlapping.length > 0) {
      ...
      const mergedRange = {
        ...
        length:
          mergedEnd.startIndex + mergedEnd.length - mergedStart.startIndex,
  ...
  function getOverlappingAnnotations(annotations, newAnnotation) {
    const partitioned = _.partition(annotations, other => {
      ...
      return (
        (newAnnotation.startIndex >= other.startIndex &&
          newAnnotation.startIndex <= otherEnd) ||
